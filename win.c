...
// Concatenates the provided list of strings into a single buffer.
char*
...
// Buffered reading from a plan9 filesystem connection.
// It reads one character at a time with buffering.
int
getec(CFid *efd)
{
...
//  Reads an integer from the provided 9p connection.
// Gets angry if the input stream is not an integer.
int
geten(CFid *efd)
{
	int n, c;
...
// Slurps a rune out of the provided 9p connection.
int
...
// Reads an Acme event from the provided 9p connection.
void
...
// length of text if provided.
	e->nr = geten(efd);

// double check that event is obeying acme rules
	if(e->nr > EVENTSIZE)
...
// stuff the runes from the text into the event struct
// we keep the utf8 and rune versions?
	for(i=0; i<e->nr; i++){
...
// Given a character buffer of length nb, compute its length 
// in runes.
int
...
stdinproc(void *v)
{
	...
	for(;;){
		if(debug)
			fprint(2, "typing[%d,%d)\n", q.p, q.p+ntyper);
// Get an acme event from the efd descriptor.
		gete(efd, &e);
		if(debug)
...
// over each  event:
		switch(e.c1)
                     // be backwards compatiable: ignore additional future messages.
		default:
		Unknown:
			print("unknown message %c%c\n", e.c1, e.c2);
			break;
    // Different external command writes to the body contents of the watched window.
...
// Keyboard or Mouse actions that edit the file
		case 'K':
		case 'M':
    // switch over the sub-type of the event.
			switch(e.c2){
         // text inserted into the body (This is a capital i)
			case 'I':
    ...
    // button 2 in the tag or body
        ...
        // button 3, tag or body
			...
			case 'L':
				...
				fsfidprint(efd, "%c%c%d %d\n", e.c1, e.c2, e.q0, e.q1);
				break;
        // text deleted or inserted into the tag.
			case 'd':
			case 'i':
				break;
...
// receives content from the child process
void
stdoutproc(void *v)
{
	int fd1 = rcfd;
...
// accumulates the specified buffer (probably from an event) into a buffer.
void
...
// determines if the fd0 has been switched to raw
// I'm thinking that fd0 here is the end point of the pty.
int
israw(int fd0)
{
...
// split the typing buffer up into lines and ship them to the
// child process.
void
sendtype(int fd0)
{
...
// Handle typing at the end of the buffer.
void
type(Event *e, int fd0, CFid *afd, CFid *dfd)
{
	...
	if(e->nr > 0)
		// accumulate type in a buffer.
		addtype(e->c1, e->q0-q.p, e->b, e->nb, e->nr);
	else{
		// text too big to be in event. read from acme
		m = e->q0;
		while(m < e->q1){
			n = sprint(buf, "#%d", m);
			fswrite(afd, buf, n);
...
// It's important to understand what's going on with the event protocol itself.
// are they purely notifies. i.e.: how is the default action enabled?
// this function (given an event) sticks the data from the event into the buffer.
// this is used to handle events that cause sie-effects in the buffer (injecting content.)
void
